...
export default function OrgDetailPage() {
  ...
  const loadData = useCallback(async (p) => {
    setLoading(true);
    try {
      const res = await billingApi.adminOrgDetail(orgId, p);
      setData(res.data);
    } catch {
      ...
      <div className="min-h-screen bg-slate-50">
        ...
        <header className="bg-white border-b sticky top-0 z-40">
          <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <Button variant="ghost" size="icon" onClick={() => navigate('/admin')} data-testid="org-back-btn">
                <ArrowLeft className="w-5 h-5" />
              </Button>
              <Building2 className="w-5 h-5 text-slate-400" />
              <div>
                <h1 className="text-lg font-bold tracking-tight" data-testid="org-name">{data?.org?.name || 'Организация'}</h1>
                {data?.org?.created_at && (
        ...
        {loading && !data ? (
          ...
          <main className="max-w-7xl mx-auto px-6 py-6 space-y-6">
            {/* KPI row */}
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-3" data-testid="kpi-cards">
            ...
            <Tabs value={tab} onValueChange={setTab}>
              <TabsList>
                <TabsTrigger value="stats" className="gap-1.5" data-testid="org-tab-stats">
                  <BarChart3 className="w-3.5 h-3.5" /> Динамика
                </TabsTrigger>
                <TabsTrigger value="users" className="gap-1.5" data-testid="org-tab-users">
                  <Users className="w-3.5 h-3.5" /> Пользователи
                </TabsTrigger>
                <TabsTrigger value="txns" className="gap-1.5" data-testid="org-tab-txns">
                  <History className="w-3.5 h-3.5" /> Транзакции
                </TabsTrigger>
                <TabsTrigger value="topup" className="gap-1.5" data-testid="org-tab-topup">
                  ...
                  <CardContent className="pt-5 space-y-4">
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <Input
                          type="number" min="1" step="1" placeholder="Например: 500"
                      ...
                      <Button onClick={handleTopup} disabled={topupLoading || !topupAmount} className="w-full gap-2" data-testid="admin-topup-btn">
                        {topupLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <TrendingUp className="w-4 h-4" />}
                        Пополнить баланс
                      </Button>
                    </div>
                    <div className="text-xs text-muted-foreground pt-2 border-t">
                      Текущий баланс: <span className="font-medium text-foreground">{fmtNum(data.balance)} кредитов</span>
                    </div>
                  </CardContent>
                </Card>
              </TabsContent>
