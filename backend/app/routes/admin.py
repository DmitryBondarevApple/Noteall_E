...
from app.core.security import get_admin_user, get_superadmin_user, hash_password
...
async def list_users(admin=Depends(get_superadmin_user)):
    users = await db.users.find({}, {"_id": 0, "password": 0}).to_list(1000)
    return [UserResponse(**{k: u.get(k) for k in ["id", "email", "name", "role", "org_id", "created_at"]}) for u in users]
...
async def delete_user(user_id: str, admin=Depends(get_superadmin_user)):
    if user_id == admin["id"]:
...
async def update_user_role(user_id: str, role: str, admin=Depends(get_superadmin_user)):
    """Superadmin can set any role."""
    if user_id == admin["id"]:
        raise HTTPException(status_code=400, detail="Cannot change own role")
    if role not in ("user", "org_admin", "admin", "superadmin"):
