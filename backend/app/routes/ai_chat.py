...
- Первый узел НЕ ИМЕЕТ input_from (получает исходный текст автоматически).
- Все остальные узлы ОБЯЗАТЕЛЬНО должны иметь input_from — массив node_id узлов, из которых они получают данные.
- input_from определяет, откуда узел берёт входные данные. Он ДОЛЖЕН совпадать со связями edges.
- Узлы также связаны через edges: {source: "node_id", target: "node_id"}. Edges и input_from должны быть согласованы.
- Для линейной цепочки A → B → C: у B будет input_from: ["A"], у C будет input_from: ["B"].
- Для fan-out (один узел → несколько): создай несколько edges от одного source и пропиши input_from у каждого целевого узла.
...
ВАЖНО: input_from ОБЯЗАТЕЛЬНО для каждого узла кроме первого! Без input_from узел не получит данных и не сможет работать.
