...
@api_router.post("/projects/{project_id}/fragments/{fragment_id}/revert", response_model=UncertainFragmentResponse)
async def revert_fragment(
    ...
    # Determine new status: if it was auto_corrected before, restore that, else pending
    # We store the previous status in 'source' field if available
    new_status = "pending"
    if fragment.get("source") == "list":
        # This was from the uncertain list - check if word is in main text
    ...
    # Restore [word?] marker in transcript if the corrected text was applied
    transcript = await db.transcripts.find_one(
        {"project_id": project_id, "version_type": "processed"},
        {"_id": 0}
    )
    if transcript and fragment.get("corrected_text"):
        ...
        # Replace the corrected text back with [original?] marker
...
1. Исправь ошибки распознавания речи, включая имена, термины, аббревиатуры — по контексту
...
ВАЖНО: Все исправления, в которых ты не уверен на 100%, ОБЯЗАТЕЛЬНО укажи в секции "Сомнительные места" в конце. Формат:

---
Сомнительные места:
1. «исходное слово» → «исправленное слово» — пояснение (напр. "восстановлено по контексту", "возможно имя", "технический термин")
2. ...
